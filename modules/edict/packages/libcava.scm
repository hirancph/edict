...
;;;
;;; libcava.scm — cavacore DSP library as a shared library.
;;;
;;; The Caelestia shell plugin requires libcava for audio visualization.
;;; Upstream cava only builds cavacore as a static library, so we build
;;; it manually as a shared library with a pkg-config file.
;;;
;;; Adapted from nehrbash's packages/libcava.scm.

(define-module (edict packages libcava)
     ...
     (list
      ...
      #~(begin
          ...
          (let* ((src #$(package-source cava))
            ...
            ;; Write pkg-config file (named 'cava' to match cmake find)
